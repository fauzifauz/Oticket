                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 text-left">
                    ...
                    <div class="lg:col-span-8 space-y-8">
                                    ...
                                    @if($content)
                                        <div class="space-y-3">
                                            ...
                                            @if($content->type === 'textarea')
                                                <textarea id="{{ $content->key }}" name="{{ $content->key }}" rows="3" class="w-full bg-gray-50 border-gray-100 rounded-2xl py-3 px-5 sm:py-4 sm:px-6 text-sm font-bold text-gray-700 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all uppercase tracking-tight leading-relaxed">{{ $content->value }}</textarea>
                                            @else
                                                <input type="text" id="{{ $content->key }}" name="{{ $content->key }}" value="{{ $content->value }}" class="w-full bg-gray-50 border-gray-100 rounded-2xl py-3 px-5 sm:py-4 sm:px-6 text-sm font-bold text-gray-700 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all uppercase tracking-tight">
                                            @endif
                                        </div>
                                    @endif
                                @endforeach
                                    ...
                                    @foreach([1, 2, 3] as $step)
                                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4 sm:gap-6 items-start">
                                            <div class="md:col-span-2 space-y-2 sm:space-y-3">
                                                ...
                                                <input type="text" name="{{ "step_{$step}_icon" }}" value="{{ $content?->value }}" class="w-full bg-gray-50 border-gray-100 rounded-xl py-2 px-4 text-center text-lg focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all sm:py-3 sm:text-xl">
                                            </div>
                                            <div class="md:col-span-4 space-y-2 sm:space-y-3">
                                                ...
                                                <input type="text" name="{{ "step_{$step}_title" }}" value="{{ $content?->value }}" class="w-full bg-gray-50 border-gray-100 rounded-xl py-2 px-4 text-[10px] font-bold text-gray-700 focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all uppercase sm:py-3 sm:text-xs">
                                            </div>
                                            <div class="md:col-span-6 space-y-2 sm:space-y-3">
                                                ...
                                                <textarea name="{{ "step_{$step}_desc" }}" rows="2" class="w-full bg-gray-50 border-gray-100 rounded-xl py-2 px-4 text-[10px] font-bold text-gray-700 focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all uppercase leading-relaxed sm:py-3 sm:text-[11px]">{{ $content?->value }}</textarea>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                                ...
                                @foreach(['announcement_title', 'announcement_message'] as $key)
                                    ...
                                    @if($content)
                                        <div class="space-y-2 sm:space-y-3">
                                            <label for="{{ $content->key }}" class="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest px-1">{{ strtoupper($content->label) }}</label>
                                            @if($content->type === 'textarea')
                                                <textarea id="{{ $content->key }}" name="{{ $content->key }}" rows="3" class="w-full bg-gray-50 border-gray-100 rounded-2xl py-3 px-5 sm:py-4 sm:px-6 text-sm font-bold text-gray-700 focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all uppercase tracking-tight leading-relaxed placeholder:text-gray-300">{{ $content->value }}</textarea>
                                            @else
                                                <input type="text" id="{{ $content->key }}" name="{{ $content->key }}" value="{{ $content->value }}" class="w-full bg-gray-50 border-gray-100 rounded-2xl py-3 px-5 sm:py-4 sm:px-6 text-sm font-bold text-gray-700 focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all uppercase tracking-tight">
                                            @endif
                                        </div>
                                    @endif
                                @endforeach
                        ...
                        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden relative text-left">
                            ...
                            <div class="px-6 py-4 sm:px-8 sm:py-6 border-b border-gray-100 bg-gray-50">
                                <h3 class="text-[10px] sm:text-xs font-black text-gray-900 uppercase tracking-widest flex items-center gap-2">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8">
                                    @foreach(['footer_company_name', 'footer_text'] as $key)
                                        ...
                                        @if($content)
                                            <div class="space-y-2 sm:space-y-3">
                                                <label for="{{ $content->key }}" class="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest px-1">{{ strtoupper($content->label) }}</label>
                                                <input type="text" id="{{ $content->key }}" name="{{ $content->key }}" value="{{ $content->value }}" class="w-full bg-gray-50 border-gray-100 rounded-2xl py-3 px-5 sm:py-4 sm:px-6 text-sm font-bold text-gray-700 focus:ring-4 focus:ring-gray-900/10 focus:border-gray-900 transition-all uppercase tracking-tight">
                                            </div>
                                        @endif
                                    @endforeach
                                </div>
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
                                    @foreach(['footer_privacy_link', 'footer_terms_link', 'footer_contact_link'] as $key)
                                        ...
                                        @if($content)
                                            <div class="space-y-2 sm:space-y-3">
                                                <label for="{{ $content->key }}" class="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest px-1">{{ strtoupper($content->label) }}</label>
                                                <input type="text" id="{{ $content->key }}" name="{{ $content->key }}" value="{{ $content->value }}" class="w-full bg-gray-50 border-gray-100 rounded-xl py-2 px-4 text-[10px] font-bold text-indigo-600 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all sm:py-3">
                                            </div>
                                        @endif
                                    @endforeach
                                </div>
